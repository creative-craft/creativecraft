...
$(document).ready(function() {
	...
	if($('.videoRow').size() > 0) {
		$('.videoRow').bind('click', function () {
			...
			var width = $('#FeaturedVideo').data('video-width') || 320;
			var height = $('#FeaturedVideo').data('video-height') || 265;
			$('#FeaturedVideo').html('<object width="' + width + '" height="' + height + '">'
				+ '<param name="movie" value="//www.youtube.com/v/' + videoId + '?fs=1"></param>'
				+ '<param name="allowFullScreen" value="true"></param>'
				+ '<param name="allowscriptaccess" value="always"></param>'
				+ '<embed src="//www.youtube.com/v/'  + videoId + '?&fs=1&autoplay=1" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="' + width + '" height="' + height + '" wmode="transparent"></embed>'
				+ '</object>'
			);
	...
	$(".VariationSelect").change(function() {
		...
		$.getJSON(
			'../remote1e0f.html?w=GetVariationOptions&amp;productId=' + productId + '&options=' + optionIds,
			function(data) {
	...
	$('.ProductOptionList input[type=radio]').click(function() {
		...
		// request values for this option
		$.getJSON(
			'../remote1e0f.html?w=GetVariationOptions&amp;productId=' + productId + '&options=' + optionId,
			function(data) {
				if (!data) {
					return;
				}
