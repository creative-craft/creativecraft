...
function fastCartAction(event) {
	...
	if (typeof(event) == 'string') {
		...
		if (!url || url.indexOf('https://store-36xna.mybigcommerce.com/cart.php') == -1) {
			return false;
		}
...
function isc_TrackSearchClick (searchId) {
	...
	var trackurl = 'searchc3a4.html?action=tracksearchclick&amp;searchid=' + encodeURIComponent(searchId) + '&random=' + Math.random();
		...
		$('.Rating img').each(function() {
			if($(this).height() == 0) {
				$(this).load(function() {
					...
					var imgName = $(this).attr('src').split('index.html');
